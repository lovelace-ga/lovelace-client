...
var landingPageView = function landingPageView() {
  $('#site-list').show();
  $('#sign-in-div').show();
  $('#sign-up-div').hide();
  $('#app-nav-bar').show();
...
var createPostView = function createPostView() {
  $('#site-list').hide();
  $('#sign-in-div').hide();
  $('#sign-up-div').hide();
  $('#app-nav-bar').show();
...
var createPageView = function createPageView() {
  $('#site-list').hide();
  $('#sign-in-div').hide();
  $('#sign-up-div').hide();
  $('#app-nav-bar').show();
...
var editPostView = function editPostView() {
  $('#site-list').hide();
  $('#sign-in-div').hide();
  $('#sign-up-div').hide();
  $('#app-nav-bar').show();
...
var editPageView = function editPageView() {
  $('#site-list').hide();
  $('#sign-in-div').hide();
  $('#sign-up-div').hide();
  $('#app-nav-bar').show();
...
var createSiteView = function createSiteView() {
  $('#site-list').hide();
  $('#sign-in-div').hide();
  $('#sign-up-div').hide();
  $('app-nav-bar').show();
...
var publicView = function publicView() {
  $('#site-list').hide();
  $('#sign-in-div').hide();
  $('#sign-up-div').hide();
  $('#app-nav-bar').show();
...
var config = {
  apiOrigins: {
    ...
    production: 'https://glacial-mountain-67409.herokuapp.com'
  }
};
...
var getOneSite = function getOneSite(id) {
  return $.ajax({
...
var createSite = function createSite(data) {
  return $.ajax({
    ...
    data: {
      site: data // data will come from get form fields,
...
var destroySite = function destroySite() {
  return $.ajax({
...
var siteAjax = __webpack_require__(7);
var landingUi = __webpack_require__(14);
// const postAjax = require('../AJAX/postajax')

var ui = __webpack_require__(43);

var loadSite = function loadSite(siteId) {
  siteAjax.getOneSite(siteId).then(ui.loadSiteSuccess).then(ui.showPublicPages).then(ui.showPublicPosts).catch(ui.loadSiteFailure); // need error handling
};

var readMore = function readMore(event) {
  var newContent = '';
  var postId = event.target.dataset.id;
  var siteId = store.publicSite.id;
  siteAjax.getOneSite(siteId).then(function (siteData) {
    return siteData.site;
  }).then(function (data) {
    var blogArray = data.blog;
    blogArray.forEach(function (post) {
      if (post.id === postId) {
        // finds post with id matching data-id from read more button
...
var onReturnToDash = function onReturnToDash(event) {
  if (!store.site) {
...
var listAllSites = function listAllSites() {
  // event.preventDefault() // I think this isn't needed
  siteAjax.getAllSites().then(landingUi.getSitesSuccess).catch(landingUi.getSitesFailure);
};

var addHandlers = function addHandlers() {
  $('#return-to-landing').on('click', function () {
    listAllSites();
...
var pageApi = __webpack_require__(21); // UPDATE THIS (change file name)
var postApi = __webpack_require__(22); // UPDATE THIS
var authApi = __webpack_require__(12);
var siteApi = __webpack_require__(7);
...
var onPublishNewPost = function onPublishNewPost(event) {
  var data = getFormFields(event.target);
  // console.log('onPublishNewPost data is', data)
  event.preventDefault();
  postApi.create(data).then(ui.publishPostSuccess).then(onGetPosts).catch(ui.publishPostFailure);
};

var onCancelNewPost = function onCancelNewPost(event) {
  event.preventDefault();
  $('#create-post-view input').val(null);
  $('#create-post-view textarea').val(null);
  onGetPosts();
...
var onNewPage = function onNewPage(event) {
  event.preventDefault();
  views.createPageView();
};

var onPublishNewPage = function onPublishNewPage(event) {
  var data = getFormFields(event.target);
  // console.log('onPublishNewPost data is', data)
  event.preventDefault();
  pageApi.create(data).then(ui.publishPageSuccess).then(onGetPages).then(function () {
    $('#error-success-msg').text('Your Page Was Created Successfully');
  }).catch(ui.publishPageFailure);
};

var onCancelNewPage = function onCancelNewPage(event) {
  event.preventDefault();
  $('#create-page-view input').val(null);
  $('#create-page-view textarea').val(null);
  onGetPages();
...
var onGetPosts = function onGetPosts() {
  if (event !== undefined) {
    event.preventDefault();
  }
  var siteID = store.site._id;
  // console.log('site id is', store.site._id)
  siteApi.getOneSite(siteID).then(function (site) {
...
var onUpdatePost = function onUpdatePost(event) {
  ...
  postApi.update(data).then(ui.updatePostSuccess).then(onGetPosts).catch(ui.updatePostFailure);
};

var onCancelUpdatePost = function onCancelUpdatePost(event) {
  event.preventDefault();
  onGetPosts();
};
...
var onDeletePost = function onDeletePost(event) {
  event.preventDefault();
  var postForDelete = event.target.dataset.id;
  postApi.destroy(postForDelete).then(onGetPosts).then(function () {
...
var onViewPage = function onViewPage(event) {
  ...
  var pagesArray = store.site.pages;
...
var onUpdatePage = function onUpdatePage(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  // console.log('onUpdatePost data is ', data)
  pageApi.update(data).then(ui.updatePageSuccess).then(onGetPages).catch(ui.updatePageFailure);
};

var onCancelUpdatePage = function onCancelUpdatePage(event) {
  event.preventDefault();
  onGetPages();
};
...
var onDeletePage = function onDeletePage(event) {
  event.preventDefault();
  var pageForDelete = event.target.dataset.id;
  pageApi.destroy(pageForDelete).then(onGetPages).then(function () {
...
var onChangePassword = function onChangePassword(event) {
  ...
  authApi.changePassword(data).then(ui.changePasswordSuccess).catch(ui.changePasswordFailure);
};

var onChangeSiteName = function onChangeSiteName(event) {
  var data = getFormFields(event.target);
  // console.log('onChangeSiteName data is', data)
  event.preventDefault();
  // console.log('onChangeSiteName store.site.id is', store.site._id)
  siteApi.updateSite(data).then(ui.changeSiteNameSuccess).then(function () {
    return siteApi.getOneSite(store.site._id);
  }).then(ui.getOneSiteSuccess).catch(ui.changeSiteNameFailure).catch(ui.getOneSiteFailure);
};

var onViewSettings = function onViewSettings(event) {
  event.preventDefault();
  views.settingsView();
};

var onDeleteSite = function onDeleteSite(event) {
  event.preventDefault();
  siteApi.destroySite().then(ui.destroySiteSuccess).then(function () {
    return views.createSiteView();
  }).catch(ui.destroySiteFailure);
};

var onCreateSite = function onCreateSite(event) {
  var data = getFormFields(event.target);
  // console.log('onCreateSite data is', data)
  event.preventDefault();
  siteApi.createSite(data.site).then(ui.createSiteSuccess).catch(ui.createSiteFailure);
};
var addHandlers = function addHandlers() {
  // New Post
...
var store = __webpack_require__(1);
var siteListTemplate = __webpack_require__(42);

var getSitesSuccess = function getSitesSuccess(data) {
  store.sites = data.sites;
  var showSiteList = siteListTemplate({ sites: data.sites });
...
var siteAjax = __webpack_require__(7);
var landingUi = __webpack_require__(14);
var visitorEvents = __webpack_require__(8);
...
var addHandlers = function addHandlers() {
  $(document).on('click', '.site-link', function (event) {
    var siteId = event.target.dataset.id;
...
var setAPIOrigin = __webpack_require__(19);
var config = __webpack_require__(4);
var dashEvents = __webpack_require__(10);
var authEvents = __webpack_require__(46);
var landingEvents = __webpack_require__(15);
var visitorEvents = __webpack_require__(8);
var views = __webpack_require__(3);
...
var parseNestedQuery = __webpack_require__(20);
...
/*
  ...
  client remote, api local
    https://ga-wdi-boston.github.io/lovelace-client/?environment=development
    This will require allowing "unsafe scripts" in Chrome
  client remote, api remote
    https://ga-wdi-boston.github.io/lovelace-client/
*/
...
var store = __webpack_require__(1);
var views = __webpack_require__(3);
var postListTemplate = __webpack_require__(24);
var pageListTemplate = __webpack_require__(41);
var publicEvents = __webpack_require__(8);

var publishPostSuccess = function publishPostSuccess(data) {
...
var publishPostFailure = function publishPostFailure() {
  $('#error-success-msg').text('Something went wrong. New post could not be created.');
};

var publishPageSuccess = function publishPageSuccess(data) {
...
var publishPageFailure = function publishPageFailure() {
...
var getPostsSuccess = function getPostsSuccess(data) {
  var showList = postListTemplate({ ps: data.site.blog });
...
var getPostsFailure = function getPostsFailure() {
...
var viewPostContent = function viewPostContent(viewPost) {
  views.editPostView();
  $('#edit-post-title').val(viewPost.title);
  $('#edit-post-content').val(viewPost.content);
};
var editPostSuccess = function editPostSuccess(data) {
  // insert handlbars here
  $('#error-success-msg').text('Your Page Was Created Successfully');
};

var editPostFailure = function editPostFailure() {
...
var updatePostFailure = function updatePostFailure(data) {
  $('#error-success-msg').text('Something went wrong. Could not update post.');
};

var getPagesSuccess = function getPagesSuccess(site) {
  // console.log('pages is', site.pages)
  var showList = pageListTemplate({ pages: site.pages });
  $('#dash-list-container').html(showList);
...
var getPageFailure = function getPageFailure() {
...
var viewPageContent = function viewPageContent(viewPage) {
  views.editPageView();
  $('#edit-page-title').val(viewPage.title);
  $('#edit-page-content').val(viewPage.content);
};

var editPageSuccess = function editPageSuccess(data) {
  // insert handlbars here
...
var updatePageFailure = function updatePageFailure(data) {
...
var changeSiteNameSuccess = function changeSiteNameSuccess(data) {
  $('#error-success-msg').text('Name Changed Successfully');
  // Show success message to user
};

var changeSiteNameFailure = function changeSiteNameFailure() {
  $('#error-success-msg').text('Error on Name Change, please try again.');
  // Show success message to user
};

var getOneSiteSuccess = function getOneSiteSuccess(data) {
...
var getOneSiteFailure = function getOneSiteFailure() {
  // console.log('getOneSiteFailure error is', error)
  // Show success message to user
};

var destroySiteSuccess = function destroySiteSuccess() {
  store.site = null;
  $('#navbar-header').text('');
...
var destroySiteFailure = function destroySiteFailure() {
...
module.exports = {
  ...
  publishPostFailure: publishPostFailure,
  publishPageSuccess: publishPageSuccess,
  publishPageFailure: publishPageFailure,
  ...
  editPostFailure: editPostFailure,
  ...
  changeSiteNameSuccess: changeSiteNameSuccess,
  changeSiteNameFailure: changeSiteNameFailure,
  getOneSiteSuccess: getOneSiteSuccess,
  getOneSiteFailure: getOneSiteFailure,
  destroySiteSuccess: destroySiteSuccess,
  destroySiteFailure: destroySiteFailure,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.ps : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
  ...
  return "      <div class=\"list-group-item\">\n        <h3 class=\"list-group-item-heading\">"
    ...
    + " type=\"button\" class=\"edit-post btn btn-primary\">edit </button>\n        <button data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + " type=\"button\" class=\"delete-post btn btn-danger\" data-toggle=\"modal\" data-target=\"#delete-post-modal\">\n          Delete Post\n        </button>\n      </div>\n      <br>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "<h4 class=\"list-group-item-heading\">This site doesn't have any blog posts yet.</h4>\n<p class=\"list-group-item-text\"> You can create one by clicking the buttons on the left</p>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.ps : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "");
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.pages : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;
  ...
  return "      <div class=\"list-group-item\">\n        <h3 class=\"list-group-item-heading\">"
    ...
    + "</p>\n        <button data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + " type=\"button\" class=\"edit-page btn btn-primary\">edit </button>\n        <button data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + " type=\"button\" class=\"delete-page btn btn-danger\" data-toggle=\"modal\" data-target=\"#delete-page-modal\">\n          Delete Page\n        </button>\n      </div>\n      <br>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "<h4 class=\"list-group-item-heading\">This site doesn't have any pages yet.</h4>\n<p class=\"list-group-item-text\" > You can create one by clicking the buttons on the left</p>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.pages : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.sites : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;
  ...
  return "  <a data-id="
    ...
    return "<h4>We don't have any sites yet to show you</h4>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "\n<h3>Take a Look</h3>\n<p>Visit are some sites that were created with our platfom</p>\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.sites : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
var publicPostsTemplate = __webpack_require__(44);
var publicPagesTemplate = __webpack_require__(45);
var views = __webpack_require__(3);
var store = __webpack_require__(1);

var loadSiteSuccess = function loadSiteSuccess(siteData) {
  $('.navbar-brand').text(siteData.site.name);
  store.publicSite = siteData.site;
  return siteData.site;
};
var showPublicPages = function showPublicPages(data) {
  // console.log('data is', data)
  var showPageList = publicPagesTemplate({ pages: data.pages });
  $('#public-nav-list').html(showPageList);
  views.publicView();
  $('.read-less').hide();
  return data;
};
var showPublicPosts = function showPublicPosts(data) {
  var blogArray = data.blog;
  blogArray.forEach(function (item) {
    item['content'] = item['content'].substring(0, 150); // trims the content before sending to handlebars
  });
  var showPostList = publicPostsTemplate({ posts: data.blog });
  $('#public-posts').html(showPostList);
  views.publicView();
  $('.read-less').hide();
};
var loadSiteFailure = function loadSiteFailure() {
  $('#error-success-msg').text('sorry there was an error loading posts for this site.');
};
var showPageContent = function showPageContent(pageData) {
  $('#public-posts').hide();
  $('#site-page-header').text(pageData.title);
  $('#site-page-content').html("<p style='white-space: pre-wrap'>" + pageData.content + '</p>');
};
module.exports = {
  loadSiteSuccess: loadSiteSuccess,
  showPublicPosts: showPublicPosts,
  loadSiteFailure: loadSiteFailure,
  showPublicPages: showPublicPages,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.posts : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "  <div class=\"list-group-item blog-post\">\n    <h2 class=\"list-group-item-heading\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.title : stack1), depth0))
    + "</h3>\n    <p class=\"list-group-item-text\" style=\"white-space: pre-wrap;\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.content : stack1), depth0))
    + "</p>\n    <a data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + " class=\"read-more\">read more</a>\n    <a data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + " class=\"read-less\">read less</a>\n  </div>\n  <br>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "<h4>This site doesn't have any blog posts yet.</h4>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.posts : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<li class='page-button' role=\"presentation\"><a data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + ">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.title : stack1), depth0))
    + "</a></li>\n";
  ...
  return "<li role=\"presentation\"><a id='blog-button'>Blog</a></li>\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.pages : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
var apiSite = __webpack_require__(7);
...
var onSignUp = function onSignUp(event) {
  ...
  }).then(ui.signInSuccess).then(function () {
    return apiSite.createSite(siteData);
  }).then(ui.createSiteSuccess).catch(ui.signUpFailure).catch(ui.signInFailure).catch(ui.createSiteFailure);
};
...
var onSignIn = function onSignIn(event) {
  ...
  }).catch(ui.signInFailure).catch(ui.getSitesFailure);
};
